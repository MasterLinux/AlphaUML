...
dojo.require("ui.dialog.EditorSearch");
...
/*
    ...
    saveFile: function() {
        //save if necessary
        if(air && this.hasChanged && this.filePath) {
        ...
        //store new file without a path -> open a dialog
        else if(air && !this.filePath) {
            //get project folder path
            var projectPath = null, file = null;
            var fileTree = this.getGlobal("filetree", null, "master");
            if(fileTree) projectPath = fileTree.getProjectPath();

            //get initial folder to starts browsing in it
            if(projectPath) file = new air.File(projectPath);
            else file = air.File.documentsDirectory;

            try {
                file.browseForSave("Save As");
                file.addEventListener(air.Event.SELECT, dojo.hitch(this, function(event) {
                    this.filePath = event.target.url;
                    this.hasChanged = true;
                    this.saveFile();
                    this.tab.setAccent(false);

                    //set new tab name
                    var name = null;
                    if(fileTree) name = fileTree.getFileName(event.target);
                    if(name) this.tab.setTitle(name, true);

                    //TODO update filetree
                    if(fileTree) fileTree.update();
                }));
            }
            catch (error) {
    ...
    openSearchDialog: function() {
        if(!this.dialog) {
            this.dialog = new ui.dialog.EditorSearch({
