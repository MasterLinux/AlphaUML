...
dojo.declare("ui.Frame", ui.Object, {
    ...
    /**
     * shows a message above the content div
     * @param msg string
     * @param consoleMsg boolean if true the message will be drawn into the console
     * @param consoleMsgType string "error" or "warning"
     */
    showMessage: function(msg, consoleMsg, consoleMsgType) {
        //remove old message
        this.removeMessage();

        //create new message id
        this.messageId = this.htmlId + "Message";

        dojo.place(
            '<div class="frameMessage" id="' + this.messageId + '">' + msg + '</div>',
            dojo.byId("main")
        );

        //register onClick events
        this.connect({
            name: "MessageOnClickEvent",
            event: "onclick",
            nodeId: this.messageId,
        ...
        //draw as console message
        if(consoleMsg) {
            var con = this.getGlobal("console", null, "master");
            if(con) con.addMessage(msg, true, consoleMsgType);
        }
    },
    ...
    removeMessage: function() {
        if(this.messageId && dojo.byId(this.messageId)) {
            dojo.destroy(this.messageId);
        } this.messageId = null;

        //remove event listener
        if(this.connects && this.connects["MessageOnClickEvent"]) {
            dojo.disconnect(this.connects["MessageOnClickEvent"].handler);
            delete this.connects["MessageOnClickEvent"];
        }
    },
    ...
    activateMenu: function() {
        ...
        //show menu
        var menuNode = dojo.byId(this.menuId);
        dojo.style(menuNode, "display", "");
    },
    ...
    deactivateMenu: function() {
        ...
        //hide menu
        var menuNode = dojo.byId(this.menuId);
        dojo.style(menuNode, "display", "none");
    },
