...
dojo.require("ui.classDiagram.JavaDoc");
                ...
                dojo.forEach(file["classes"], dojo.hitch(this, function(c) {
                    ...
                    if(javaDoc) {
                        ...
                        if(_desc) {
                            ...
                            description = description.replace(/\s*/, "") !== "" ? description : null;
                        }
                        ...
                        //default javadoc params
                        var _atParam = javaDoc["param"];
                        if(_atParam && _atParam.length > 0) {
                            atParam = _atParam;
                        }
                        var _atReturn = javaDoc["return"];
                        if(_atReturn && _atReturn.length > 0) {
                            atReturn = _atReturn;
                        }
                        var _atThrows = javaDoc["throws"];
                        if(_atThrows && _atThrows.length > 0) {
                            atThrows = _atThrows;
                        }
                        var _atException = javaDoc["exception"];
                        if(_atException && _atException.length > 0) {
                            atException = _atException;
                        }
                        var _atSince = javaDoc["since"];
                        if(_atSince && _atSince.length > 0) {
                            atSince = _atSince;
                        }
                        var _atAuthor = javaDoc["author"];
                        if(_atAuthor && _atAuthor.length > 0) {
                            atAuthor = _atAuthor;
                        }
                        var _atSee = javaDoc["see"];
                        if(_atSee && _atSee.length > 0) {
                            atSee = _atSee;
                        }
                    }
                    ...
                    //create javadoc storage
                    var _javaDoc = new ui.classDiagram.JavaDoc();
                    _javaDoc.atParam = atParam;
                    _javaDoc.atReturn = atReturn;
                    _javaDoc.atThrows = atThrows;
                    _javaDoc.atException = atException;
                    _javaDoc.atSince = atSince;
                    _javaDoc.atAuthor = atAuthor;
                    _javaDoc.atSee = atSee;

                    //create only if a value is set
                    if(_javaDoc.isNull()) _javaDoc = null;
                    ...
                    dojo.forEach(body["method"], dojo.hitch(this, function(m) {
                        ...
                        //create javadoc storage
                        var _javaDoc = new ui.classDiagram.JavaDoc();
                        _javaDoc.description = description;
                        _javaDoc.atParam = atParam;
                        _javaDoc.atReturn = atReturn;
                        _javaDoc.atThrows = atThrows;
                        _javaDoc.atException = atException;
