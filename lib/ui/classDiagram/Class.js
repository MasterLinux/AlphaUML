...
dojo.declare("ui.classDiagram.Class", ui.Object, {
    ...
    create: function() {
        ...
        //update each registered connector
        this.dnd.onMoved = dojo.hitch(this, function() {
            if(this.connectors) {
                for(var id in this.connectors) {
                    //get connector
                    var c = this.connectors[id];

                    if(typeof c == "object") {
                        //get name of the registered point
                        var n = this.connectors[id + "Name"];
                        //get connected side
                        var s = this.connectors[id + "Side"];
                        //update registered point
                        this.setConnectorPoint(c[n], s);
                    }
                }
            }
        });
     ...
     * @param c ui.classDiagram.Connector
     * @param pointName string the name of the connected point "p0" or "p3"
     * @param side string side on which the point will be connected "left", "right", "top" or "bottom"
     */
    registerConnector: function(c, pointName, side) {
        if(!this.connectors) this.connectors = {};

        //store connector if necessary
        if(!this.connectors[c.htmlId]) {
            this.connectors[c.htmlId] = c;
            this.connectors[c.htmlId + "Name"] = pointName;
            this.connectors[c.htmlId + "Side"] = side;

            //set point position
            this.setConnectorPoint(c[pointName], side);

            //listen for update events
            this.subscribe({
                event: c.UPDATE_EVENT,
                method: function(event) {
                    //updates the position or side
                    if(event.pointName == pointName) {
                        var side = this.connectors[c.htmlId + "Side"];
                        if(event.side != side) {
                            this.connectors[c.htmlId + "Side"] = event.side;
                            this.setConnectorPoint(c[pointName], event.side)
                        }
    ...
    /**
     * updates a connected point
     * @param p
     */
    setConnectorPoint: function(p, side) {
        var size = dojo.position(this.htmlId, true);
        var x, y;

        if(side == "top") {
            x = size.x + (size.w/2); //+- offset
            y = size.y - dojo.position(p.htmlId, true).h;
        } else if(side == "right") {
            x = size.x + size.w;
            y = size.y + (size.h/2); //+- offset
        } else if(side == "bottom") {
            x = size.x + (size.w/2); //+- offset
            y = size.y + size.h;
        } else if(side == "left") {
            x = size.x - dojo.position(p.htmlId, true).w;
            y = size.y + (size.h/2); //+- offset
        }
