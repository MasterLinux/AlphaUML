...
dojo.declare("ui.TabBar", ui.Object, {
    ...
    create: function() {
        ...
        dojo.place(
             '<div class="TabBar" uitype="' + this.uiType + '" id="' + this.htmlId + '">' +
                '<div class="leftBtn" id="' + this.leftBtnId + '"><div class="icon"></div></div>' +
                '<div class="outer" id="' + this.outerId  + '">' +
                    '<div class="inner" id="' + this.innerId + '"></div>' +
                '</div>' +
                '<div class="rightBtn" id="' + this.rightBtnId + '"><div class="icon"></div></div>' +
            '</div>',
        ...
        if(this.unregTabs) {
            ...
            var tabIndex = 0;
            var firstTab = '';
            for(var title in this.unregTabs) {
                var tab = this.unregTabs[title];
    ...
    addTab: function(tab, activate, replace) {
        ...
        //replace content
            ...
            if(tab.menu) tab.menu.place(frame);
            tab.content.place(frame);
        }
        ...
        //store new tab
        if(!this.tabs[tab.htmlId]) {
            this.tabs[tab.htmlId] = tab;
        } else {
            if(air) air.trace("tab duplicate: " + tab.title);
            this.tabs[tab.htmlId].close();
            this.tabs[tab.htmlId] = tab;
        }
    ...
    removeTab: function(id) {
        //TODO if the last tab will remove. destroy tabBar
    ...
    getTab: function(filter) {
        ...
        //index is given
            ...
            for(var id in this.tabs) {
                ...
                if(this.tabs[id].index === filter) {
                    resultTab = this.tabs[id];
                    isRegistered = true;
                    //break this loop
                    break;
                };
        ...
        //get active tab
            ...
            for(var id in this.tabs) {
                //search for tab with the given index
                if(this.tabs[id].isActivated === filter) {
                    resultTab = this.tabs[id];
                    isRegistered = true;
                    //break this loop
                    break;
    ...
    width: function() {
        ...
        var nodeUtil = new ui.util.Node();
        //return width of this tab bar
        return nodeUtil.width(this.htmlId);
    },
    ...
    height: function() {
        ...
        return nodeUtil.height(this.htmlId);
    }
});
