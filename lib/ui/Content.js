...
dojo.declare("ui.Content", ui.Object, {
    ...
    setSize: function(width, height) {
        ...
        var parentPos = (parent !== null || parent !== undefined) ? dojo.position(parent) : {};

        this.width = width || parentPos.w || 0;
        this.height = height || parentPos.h || 0;

        //set the containers size
        if(this.contentNode && !this.lockResizing) {
    ...
    place: function(frame) {
        ...
        if(frame) {
            ...
            //execute start handler if set
            if(this.onPlaceStart) this.onPlaceStart();
        ...
        if(frame) {
            //execute end handler if set
            if(this.onPlaceEnd) this.onPlaceEnd();
